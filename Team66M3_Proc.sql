...
update Product 
set customer_username =@customername , available ='False'
...
if @totalamount= @credit --pays totally in credit
begin
...
if @totalamount =@cash  --pays totally in cash
begin
...
If @credit is not null and @cash is NULL and @totalamount>@credit  and   (DATEDIFF(DAY, GETDATE(), @edate)>0) 
...
if @cash is not NULL and @credit is NULL and @totalamount>@cash and (DATEDIFF(DAY, GETDATE(), @edate)>0) 
--pays partially in credit
begin
update Orders set Gift_Card_code_used=@gccused
where order_no=@orderID
